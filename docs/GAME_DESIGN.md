...
5. **Settings** — sound, haptics, reduce-motion, theme override (auto/light/dark), and (P1+) account/cloud-sync if ever added.
...
| **Color blindness** | Handled by default, with no separate mode or toggle to remember to turn on: Fair vs. True is never encoded by hue alone — the bands are also different widths (§3.2), and the share-card glyphs are distinct shapes (`○ ◐ ●`), not just colors. An earlier draft added a dedicated "Colorblind-Safe Mode" toggle whose only effect was a rectangular outline drawn around the canvas — it didn't actually change how the bands themselves were distinguished, so it was removed rather than kept as a setting that looked like it did something it didn't. |
