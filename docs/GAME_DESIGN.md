...
|---|---|
| **Contrast** | Both light and dark themes are tuned to meet WCAG 2.1 AA contrast ratios (4.5:1 for text, 3:1 for the essential graphical distinction between ring, pointer, and target bands) — verified as an explicit QA pass (PRODUCT_PLAN.md → Testing Strategy), not assumed from a color picker. |
