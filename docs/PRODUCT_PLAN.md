...
- `src/render.js` — canvas 2D rendering, reduced-motion-aware.
...
- **Unit tests** for the deterministic core: the PRNG, the daily-seed derivation (same UTC date → same seed, always), the hit-detection math (given a known sweep state and a known input timestamp, the judged result is exact and reproducible) — this is the fairness-critical path flagged repeatedly in validation, and it is the cheapest place in the whole project to catch a fairness bug.
- **Playwright end-to-end tests**, run headlessly in this environment: page loads without console errors, a simulated tap sequence produces the expected miss/fair/perfect sequence, settings persist across a reload, the share-card text is generated correctly for a given seeded result, offline mode works (service worker serves the app shell with network disabled).
- **Cross-device manual QA matrix**: at minimum one flagship iOS device, one flagship Android device, and one budget Android device (2–3 year old, sub-$200 tier), specifically to catch the touch-sampling-rate and audio-latency fairness issues named in validation before they reach real users.
...
| **5 — MVP** | Add the Daily Ring, streak, Precision Journal, share card, settings/accessibility — the full P0 scope (see Prioritization) | This document's full P0 feature list, shippable as a PWA | All P0 features present; automated Playwright suite passes; manual accessibility pass (contrast, colorblind sim, reduced-motion, sound-off) passes | Scope creep into P1 features (weekly modifiers, server leaderboard) — actively resist | Proceed to internal testing once P0 is complete and P1 items are confirmed absent, not "coming soon" |
...
| **8 — Soft Launch** | Get real, unbiased retention/virality data at small scale, before any wider release or native investment | Public web build, no paid promotion, 1–2 modest organic-only launch touchpoints (e.g., a niche community post, a small press pitch) | The Analytics KPI targets (PRODUCT_PLAN.md → Analytics): D1/D7/D30 retention and — the single most important number — share rate | This is the phase that actually answers the concept's central open question; a disappointing result here is real information, not a testing artifact to explain away | **This is the concept's real go/no-go gate.** If share rate and D7 retention land meaningfully below target with no clear, fixable cause, do not proceed to native investment — revisit positioning or return to Phase 1 rather than sinking further budget |
...
The MVP is precisely the P0 list above — restated here as a single acceptance checklist for "is this done":

- [ ] A player can open the app (or PWA) cold, with zero prior instruction, and complete a correctly-judged first lap within 10 seconds, guided only by the single first-run hint pulse.
- [ ] The Daily Ring is identical for every player on the same UTC calendar date, fully playable offline, with no server dependency.
- [ ] Practice mode is available, clearly secondary to the Daily Ring on the Home screen.
- [ ] Hit detection is judged against the raw input event timestamp, verified correct by unit tests against known seed→outcome pairs.
- [ ] The sweep is non-periodic (not constant angular velocity) by default.
- [ ] Difficulty ramps on exactly two knobs (speed, band width) up to a hard, tuned plateau.
- [ ] A Fair result halves rather than zeroes the combo multiplier; only a Miss ends a run.
- [ ] The Precision Journal persists locally and displays median offset, consistency, longest streak, and total Perfects.
- [ ] A streak counter persists locally, counts participation (not performance), and a small number of freezes are earned (never purchasable).
- [ ] The Result screen offers a plain text/emoji share card via Web Share API with a clipboard fallback — no rendered image, no extra confirmation step.
- [ ] Settings include independent Sound, Haptics, Reduce Motion, Colorblind-Safe Mode, and Theme (auto/light/dark) toggles, all applying instantly.
- [ ] The app is installable as a PWA and fully playable offline after first load.
- [ ] Zero ads, zero IAP, zero server backend, zero server leaderboard are present anywhere in the build.
- [ ] The full automated (Playwright + unit) test suite passes, and the manual accessibility/device-tier QA matrix has been run at least once.
